...
TEST_F(CollisionTest, happy_day) {
    ...
    main_loop.tick(1.0);
    std::vector<GMCpCollider*> collided = collider->get_collided_with_others();
    ASSERT_EQ(collided.size(), 1);
    ASSERT_TRUE(std::find(collided.begin(), collided.end(), obj2->get_collider()) != collided.end());
}